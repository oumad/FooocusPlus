...
def refresh_output_list(max_per_page, max_catalog):
    ...
    print(f'[Gallery] Refresh Output Catalog: A total of {total_nums} images and {pages} pages, displaying the latest {pages if pages<display_max_pages else display_max_pages} pages.')
...
def select_index(choice, image_tools_checkbox, state_params, evt: gr.SelectData):
    ...
    print(f'[Gallery] Selected Gallery Catalog: change image catalog:{choice}.')
...
def select_gallery(choice, state_params, backfill_prompt, evt: gr.SelectData):
    ...
    #print(f'[Gallery] Selected Gallery: selected index {evt.index} of {choice} images list:{result["Filename"]}.')
...
def refresh_images_catalog(choice: str, passthrough = False):
    ...
    if not passthrough and choice in images_list_keys:
        ...
        #print(f'[Gallery] Refresh Images List: hit cache {len(images_list[choice])} image items of {choice}.')
    ...
    images_list_keys.append(choice)
    parse_html_log(choice, passthrough)
    print(f'[Gallery] Refresh Images Catalog: loaded {len(images_list[choice])} image_items of {choice}.')
    return images_list[choice]
...
def get_images_prompt(choice, selected, max_per_page, display_index=False):
    ...
    if display_index:
        print(f'[Gallery] The image selected: catalog={choice}, page={page_choice}, in page={page_index}, in catalog={selected}, filename={filename}')
...
def parse_html_log(choice: str, passthrough = False):
    ...
    if not passthrough and choice in images_prompt_keys and images_prompt[choice]:
        images_prompt_keys.remove(choice)
        images_prompt_keys.append(choice)
        #print(f'[Gallery] Parse HTML Log: hit cache {len(images_prompt[choice])} image_infos of {choice}.')
        return
        ...
        if len(text)>20:
            ...
            if len(text)>10:
                ...
                if 'Upscale (Fast)' not in text:
                    print(f'[Gallery] Parse HTML Log: Parse error for {choice}, file={html_file}\ntext:{info.xpath(".//text()")}')
    ...
    print(f'[Gallery] Parse HTML Log: loaded {len(images_prompt[choice])} image_infos of {choice}.')
    return
