...
def get_default_base_SD3m_name():
    ...
    for i in range(dtype, -1 ,-1):
        ...
        if launch.MODELS_INFO.exists_model_key(f'checkpoints/{sd3name}'):
            return sd3name
...
def get_default_base_Flux_name(plus=False):
    ...
    for i in range(0, len(checklist)):
        ...
        if launch.MODELS_INFO.exists_model(catalog="checkpoints", model_path=fluxname):
            return fluxname
    return checklist[0]
    ...
    if task_name == 'default':
        if task_method == default_method_names[1]:
            ...
            if 'iclight_enable' in options and options["iclight_enable"]:
                if launch.MODELS_INFO.exists_model(catalog="checkpoints", model_path=default_base_SD15_name):
        ...
        comfy_params = ComfyTaskParams(default_params)
        if not launch.MODELS_INFO.exists_model(catalog="checkpoints", model_path=default_params["base_model"]):
        ...
        if base_model == 'auto':
            ...
            if not launch.MODELS_INFO.exists_model(catalog="checkpoints", model_path=base_model) and launch.MODELS_INFO.exists_model(catalog="checkpoints", model_path=model_hyp8):
        ...
        if 'nf4' in base_model.lower() and 'bnb' in base_model.lower():
        ...
        elif 'fp8' in base_model.lower() and launch.MODELS_INFO.exists_model_key(base_model_key)  and launch.MODELS_INFO.get_model_key_info(base_model_key)["size"]/(1024*1024*1024)>15:
            ...
            if 'clip_model' not in default_params or default_params['clip_model'] == 'auto':
                ...
                if not launch.MODELS_INFO.exists_model("clip", clip_model):
                    if clip_model == 't5xxl_fp16.safetensors' and launch.MODELS_INFO.exists_model("clip", 't5xxl_fp8_e4m3fn.safetensors'):
...
def check_download_kolors_model(path_root):
    ...
    if not launch.MODELS_INFO.exists_model_key(check_model_file[0]):
        load_file_from_url(
    ...
    if not launch.MODELS_INFO.exists_model_key(check_model_file[1]):
    ...
    if not launch.MODELS_INFO.exists_model_key(check_model_file[2]):
    ...
    launch.MODELS_INFO.refresh_from_path()  
    return

def check_download_base_model(base_model):
    if not launch.MODELS_INFO.exists_model(catalog="checkpoints", model_path=base_model):
        load_file_from_url(
...
def check_download_flux_model(base_model, clip_model=None):
    ...
    if clip_model:
        if not launch.MODELS_INFO.exists_model(catalog="clip", model_path=clip_model):
            load_file_from_url(
        ...
        if not launch.MODELS_INFO.exists_model(catalog="clip", model_path='clip_l.safetensors'):
            load_file_from_url(
        ...
        if not launch.MODELS_INFO.exists_model(catalog="vae", model_path='ae.safetensors'):
            load_file_from_url(
