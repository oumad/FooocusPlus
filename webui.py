...
local.gradio_root = gr.Blocks(
    title=title,
    css=topbar.css + toolbox.css).queue()

with local.gradio_root:
    ...
    with gr.Row():
                ...
                with gr.Row():
                    with gr.Column(scale=12):
                        ...
                        if isinstance(default_prompt, str) and default_prompt != '':
                            local.gradio_root.load(lambda: default_prompt, outputs=prompt)
        ...
        with gr.Column(scale=1, visible=modules.config.default_advanced_checkbox, elem_id="scrollable-box-hidden") as advanced_column:
            with gr.Tab(label='Settings', elem_id="scrollable-box"):
                ...
                local.gradio_root.load(update_history_link, outputs=history_link, queue=False, show_progress=False)
            ...
            with gr.Tab(label='Styles', elem_classes=['style_selections_tab']):
                ...
                local.gradio_root.load(lambda: gr.update(choices=copy.deepcopy(style_sorter.all_styles)),
        ...
        local.gradio_root.load(inpaint_mode_change, inputs=[inpaint_mode, inpaint_engine_state], outputs=[
                inpaint_additional_prompt, outpaint_selections, example_inpaint_prompts, 
        ...
        for mode, disable_initial_latent, engine, strength, respective_field in enhance_inpaint_update_ctrls:
            local.gradio_root.load(enhance_inpaint_mode_change, inputs=[mode, inpaint_engine_state], outputs=[
    ...
    local.gradio_root.load(fn=lambda x: x, inputs=system_params, outputs=state_topbar, _js=topbar.get_system_params_js, queue=False, show_progress=False) \
