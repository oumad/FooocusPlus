...
def get_sha256(filepath):
    ...
    if filepath not in hash_cache:
        filehash = launch.MODELS_INFO.get_file_muid(filepath)
        if not filehash:
...
class MetadataParser(ABC):
    ...
    def set_data(self, raw_prompt, full_prompt, raw_negative_prompt, full_negative_prompt, steps, base_model_name,
        ...
        if base_model_name not in ['', 'None']:
            base_model_path = launch.MODELS_INFO.get_model_filepath('checkpoints', base_model_name)
            self.base_model_hash = launch.MODELS_INFO.get_file_muid(base_model_path)

        if refiner_model_name not in ['', 'None']:
            self.refiner_model_name = Path(refiner_model_name).stem
            refiner_model_path = launch.MODELS_INFO.get_model_filepath('checkpoints', refiner_model_name)
            self.refiner_model_hash = launch.MODELS_INFO.get_file_muid(refiner_model_path)

        self.loras = []
        for (lora_name, lora_weight) in loras:
            if lora_name != 'None':
                lora_path = launch.MODELS_INFO.get_model_filepath('loras', lora_name)
                lora_hash = launch.MODELS_INFO.get_file_muid(lora_path)
